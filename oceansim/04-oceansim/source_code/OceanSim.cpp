...
private:
    ...
    void outputStatisticsSection(std::ofstream& file) {
        ...
        #pragma omp parallel for collapse(2) reduction(+:avg_temp,avg_salinity,avg_velocity,avg_pressure) reduction(min:min_temp,min_salinity,min_pressure) reduction(max:max_temp,max_salinity,max_velocity,max_pressure)
        for (int i = 0; i < width; i++) {
            for (int j = 0; j < height; j++) {
                for (int k = 0; k < depth; k++) {
                    avg_temp += temperature[i][j][k];
                    if (temperature[i][j][k] < min_temp) min_temp = temperature[i][j][k];
                    if (temperature[i][j][k] > max_temp) max_temp = temperature[i][j][k];
                    
                    avg_salinity += salinity[i][j][k];
                    if (salinity[i][j][k] < min_salinity) min_salinity = salinity[i][j][k];
                    if (salinity[i][j][k] > max_salinity) max_salinity = salinity[i][j][k];
                    ...
                    if (pressure[i][j][k] < min_pressure) min_pressure = pressure[i][j][k];
                    if (pressure[i][j][k] > max_pressure) max_pressure = pressure[i][j][k];
                }
            }
        }
...
public:
    ...
    void printStatistics() {
        ...
        #pragma omp parallel for collapse(2) reduction(+:avg_temp) reduction(max:max_velocity)
        for (int i = 0; i < width; i++) {
            for (int j = 0; j < height; j++) {
                for (int k = 0; k < depth; k++) {
                    avg_temp += temperature[i][j][k];
